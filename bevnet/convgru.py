...
class ConvGRU(nn.Module):
    ...
    def forward(self, input_tensor, pose=None, hidden_state=None):    # pose: torch.Size=([1, 6, 1, 3, 3])
        """
        ...
        if hidden_state is not None:
            ...
            hidden_state = self._init_hidden(batch_size=input_tensor.size(0))       # list: len 2   list[0].shape -> [1, 192, 512, 512]    list[1]:{NoneType}None
        ...
        seq_len = input_tensor.size(1)   # 6
        cur_layer_input = input_tensor   # torch.Size([1, 6, 192, 512, 512])

        for layer_idx in range(self.num_layers):        # self.num_layers == 1
            h = hidden_state[layer_idx]                 # shape: [1, 192, 512, 512]
            if self.use_pose:
            ...
            for t in range(seq_len):
                ...
                if self.use_pose:
                    input_pose = pose[:, t, 0]                     # torch.Size -> [1, 3, 3]
                    ## transform h to input_pose coordinate frame
                    if cell_pose is not None:                      # cell_pose torch.Size -> [1, 3, 3]
                        # cell_pose -> input_pose transofrmation
                        # inv(input_pose) * cell_pose
                        M = torch.matmul(torch.inverse(input_pose), cell_pose)[:, :2]        # shape: [1, 2, 3]    # inverse() 矩阵求逆   matmul() 矩阵乘法

                        if self.noisy_pose:            # False
                            M = self._noisify(M)
                ...
                h = self.cell_list[layer_idx](                             # 1 192 512 512
            ...
            layer_output = torch.stack(output_inner, dim=1)   # shape [1, 6, 192, 512, 512]
