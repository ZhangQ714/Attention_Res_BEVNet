...
def train_single(nets, net_opts, g):
    ...
    def validate(nets, validloader, criterion, n_iter, writer, device):
        ...
        return loss_avg, evaluator.acc()

    voxel_cfg = g.voxelizer
    voxel_generator = VoxelGenerator(
        vsize_xyz=list(voxel_cfg['voxel_size']),
        coors_range_xyz=list(voxel_cfg['point_cloud_range']),
        num_point_features=4,
        max_num_points_per_voxel=voxel_cfg['max_number_of_points_per_voxel'],
        max_num_voxels=voxel_cfg['max_voxels'])
    ...
    for epoch in range(1, g.epochs + 1):
        if resume_epoch < epoch:
            train_loss = train(nets, net_opts, trainloader, criterion, epoch, writer, g.train_device)
            _save_model(nets, net_opts, epoch, g, os.path.join(output, 'model.pth'))

            n_iter = epoch * len(trainloader)
            val_loss, val_acc = validate(nets, validloader, criterion, n_iter, writer, g.train_device)
        ...
        # 折线图
        ty = train_loss_s
        tx = range(1, len(train_loss_s) + 1)
        vx = range(1, len(val_loss_s) + 1)
        vy = val_loss_s
        vacc = val_acc_s
        plt.plot(tx, ty, label='train loss', linewidth=2, color='gold')
        plt.plot(vx, vy, label='val loss', linewidth=2, color='blue')
        plt.plot(vx, vacc, label='val acc', linewidth=2, color='green')
        plt.xlabel('train epoch')
        plt.ylabel('train&val loss & val acc')
        plt.title('Loss')
        plt.legend()
        # plt.show()
        plt.savefig(g.output + '/loss_1.png')
        plt.close()
...
def train_recurrent(nets, net_opts, g):
    ...
    def _step(nets, inputs, labels=None, criterion=None):
        ...
        if labels is not None and criterion is not None:
            labels = labels.long()        # [1, 6, 512, 512]
            t, n, c, h, w = pred.shape    # 6 1 5 512 512
            pred = pred.reshape((t, c, h, w))
            # maxpred = torch.max(pred, dim=0)
            labels = labels.squeeze(0)
            loss = criterion(pred, labels)     # 计算6个样本的平均损失
        return pred, loss
    ...
    criterion = torch.nn.CrossEntropyLoss(
        reduction="mean", ignore_index=ignore_idx, weight=class_weights)     # reduction="mean"   当有多个样本输入时，计算平均损失
    ...
    for epoch in range(1, g.epochs + 1):
        if resume_epoch < epoch:
            ...
            _save_model(nets, net_opts, epoch, g, os.path.join(output, 'model.pth'))
            n_iter = epoch * len(trainloader)
            val_loss, val_acc = _validate(nets, validloader, criterion, n_iter, writer, g.train_device)
            if val_loss < best_valid_loss:
                print("new best valid loss at %3f, saving model..." % val_loss)
                best_valid_loss = val_loss
                _save_model(nets, net_opts, epoch, g, os.path.join(output, 'best.pth'))

            train_loss_r.append(np.mean(train_loss))
            val_loss_r.append(val_loss)
            val_acc_r.append(val_acc)
        ...
        # 折线图
        ty = train_loss_r
        tx = range(1, len(train_loss_r) + 1)
        vx = range(1, len(val_loss_r) + 1)
        vy = val_loss_r
        vacc = val_acc_r
        plt.plot(tx, ty, label='train loss', linewidth=2, color='gold')
        plt.plot(vx, vy, label='val loss', linewidth=2, color='blue')
