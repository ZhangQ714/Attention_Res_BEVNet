...
class BEVNetBase(object):
    ...
    def _make_voxelizer(self, cfg):
        return VoxelGenerator(
            vsize_xyz=list(cfg['voxel_size']),
            coors_range_xyz=list(cfg['point_cloud_range']),
            num_point_features = 4,
            max_num_points_per_voxel=cfg['max_number_of_points_per_voxel'],
            max_num_voxels=cfg['max_voxels'])
...
class BEVNetSingle(BEVNetBase):
    def predict(self, points):
        with torch.no_grad():
            ...
            voxels, coords, num_points, _ = self.voxelizer.generate_voxel_with_id(points_tensor)
