...
class HardNet1024Skip(nn.Module):
    ...
    """
    def __init__(self, input_ch, n_classes=19):    # 192  5
        super(HardNet1024Skip, self).__init__()

        ch_list = [32, 64, 96, 128, 160]
        grmul = 1.7
        gr = [10, 16, 18, 24, 32]        # growth_rate
        n_layers = [4, 4, 8, 8, 8]
    ...
    def forward(self, x):
        skip_connections = []
        size_in = x.size()         # shape[6, 192, 512, 512]
        inputs = x
        ...
        for i in range(len(self.base)):
            ...
            if i in self.shortcut_layers:
                skip_connections.append(x)
        out = x               #shape(n_frame, 160, 16, 16)

        # out = out.cpu()
        # se = SEAttention(channel=160, reduction=8)
        # out = se(out)
        # out = out.cuda()

        # attention  CVAM
        # out = out.cpu()
        # cbam = CBAMBlock(channel=160, reduction=16, kernel_size=7)
        # out = cbam(out)
        # # print(111)
        # out = out.cuda()
        ...
        out = F.relu(self.final_upsample(out, output_size=(out.size(0), out.size(1)) + size_in[2:4]))        # [n_frames, 48, 512, 512]

        out = torch.cat([inputs, out], dim=1)       # [n_frames, 240, 512, 512]
        out = self.finalConv(out)         # [n_frames, 5, 512, 512]
        return out
