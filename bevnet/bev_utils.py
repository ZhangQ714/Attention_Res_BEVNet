...
class BEVLoaderV2(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        points1 = torch.from_numpy(points)
        voxels, coords, num_points= self.voxel_generator(points1)
        voxels_numpy = voxels.numpy()

        voxel_point_idxs = voxels_numpy[:, :, -1].astype(np.int32)  # num_voxels x max_num_points_per_voxel
        # voxels = voxels[:, :, :-1]
...
class BEVLoaderMultistepV3(data.IterableDataset):
    ...
    def __init__(
        ...
        """
        self.root = dataset_path
        sequences = config['sequences']            # ['00000', '00001', '00002', '00003']
        ...
        for seq in sequences:
            ...
            start += l
        self.seq_len_cumsum = np.cumsum(seq_lens)   # [2147 3766 7213 8697]   数组元素累加
    ...
    def __iter__(self):
        ...
        for start, end, stride in sequences:
            ...
            frame_idxs = np.arange(start, end, stride)    # 帧索引   len=20=seq_len  stride=frame_stride的某个值
            ...
            ## read the frame_idxs n_frame by n_frame
            for index in np.arange(0, len(frame_idxs), self.n_frame):     # len(frame_idxs)=seq_len=20
